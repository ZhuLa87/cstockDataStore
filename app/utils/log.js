...
class Log {
    ...
    static wtf(message) {
        if (this.logLevel <= LogLevels.WTF) {
            console.log(`\x1b[35m[WTF] [${this.getTimeStamp()}] ${message}\x1b[0m`);
        }
    }
    ...
    // 顯示目前的日誌級別
    static showLogLevel() {
        const levelName = Object.keys(LogLevels).find(key => LogLevels[key] === this.logLevel) || 'UNKNOWN';
        console.log(`\x1b[35m[SYSTEM] [${this.getTimeStamp()}] 當前日誌級別: ${levelName}\x1b[0m`);
    }
}
